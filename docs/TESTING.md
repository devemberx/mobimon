...
| Parking badge requires complete valid Park evidence; standard routes retain the 344 × 76 Figma icon and geometry, restricted state uses themed colors and centers the pause icon with copy, conversation retains its 258 × 60 or 272 × 60 badge; initial profile loading omits it | `ParkingBadgeStatusTest`, `MobiMonParkingBadgeTest`, and owning Home, Settings, Vehicle, Quest, Store, Connection and Conversation screen suites |
