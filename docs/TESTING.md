...
| Shared warning classification, interruptible 200ms crossfade, four-second local torso breathing, 6.5-second tired blink, fixed ground anchor, reduced motion and unchanged bounds | `build_mobi_unhealthy_sprites.py` validates alpha, canonical body preservation and eye-only changes; `VehicleConditionTest`, `MobiWarningAnimationTest`, `DecorativeMotionTest`, and `CompanionReviewTest` cover state, rendering and Home wiring |
