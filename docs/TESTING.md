...
| Parking badge requires complete valid Park evidence; loaded routes share copy, icon-free centered text, fixed size, muted orange restricted style and top-right geometry while the badge moves with its route; initial profile loading omits it | `ParkingBadgeStatusTest`, `MobiMonParkingBadgeTest`, and owning Home, Settings, Vehicle, Quest, Store, Connection and Conversation screen suites |
