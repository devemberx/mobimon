...
class CopilotConnectionJourneyTest {
    ...
    fun settingsConnectionSurvivesRecreationAndBackReturnsToSettings() {
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            ...
            compose.onNodeWithText(text(AuthR.string.copilot_connect)).ensureDisplayed().performClick()
            compose.onNodeWithText(text(AuthR.string.copilot_unavailable)).assertExists()
            scenario.recreate()
            waitFor(hasText(text(AuthR.string.copilot_unavailable)))
